...
public abstract class EnchantmentUtilsMixin {
	 ...
	 */
	@Overwrite
	public static boolean chargeExperience(PlayerEntity player, int cost) {
		if (cost < 0) return false; // consistent with original chargeExperience
		...
		int levelCost = getLevelForExperience(cost);
		int xpCostRemainder = (cost - getTotalExperienceForLevel(levelCost)); // this should be 0 for regular Anvil/Enchanting Table use
