	...
	public static void addServerConfigTask(GatherLoginConfigurationTasksEvent event) {
		event.addTask(new SimpleConfigurationTask(TaxFreeLevelsPacketHandler.KEY, context -> {
			LogicalSidedProvider.WORKQUEUE.get(LogicalSide.SERVER).execute(() -> {
				if (NetworkContext.get(context.getConnection()).getRemoteChannels().contains(ServerConfigPayload.ID)) {
					TaxFreeLevelsPacketHandler.INSTANCE.send(TaxFreeLevelsConfig.LOCAL_CONFIG.get(), context.getConnection());
				}
			});
		}));
	}
					...
					if (client.isInSingleplayer()) {
						...
						if (server != null) {
							for (var player : server.getPlayerManager().getPlayerList()) {
								if (NetworkContext.get(player.networkHandler.getConnection()).getRemoteChannels().contains(ServerConfigPayload.ID)) {
									TaxFreeLevelsPacketHandler.INSTANCE.send(TaxFreeLevelsConfig.LOCAL_CONFIG.get(), player.networkHandler.getConnection());
								}
							}
						}
					}
